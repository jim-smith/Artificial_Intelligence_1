...
def make3dscatters(feature_values,labels):
    ...
    panecolor= (0.5,0.5,0.5,0.3) # background to make tyhings visible

    ax1.scatter(feature_values[:,:1], feature_values[:,1:2],feature_values[:,2:3],c=colours, marker='o',s=100,label=labels)
    ax1.set_xlabel('Red')
    ...
    ax2.scatter(feature_values[:,3:4], feature_values[:,4:5],feature_values[:,5:6],c=colours, marker='o',s=100,label=labels)
...
def get_distances(cities):
    #how many rows re there in the lsit of city co-ordinates?
